...
<div class="card overflow-hidden p-4 article-wrapper" data-aos="fade-zoom-in">
  <a href="<?= $url ?>" class="article-thumbnail <?php $image === gambar_desa($desa['logo']) and print('flex items-center') ?>">
    <img src="<?= $image ?>" alt="logo desa" class="<?php $image !== gambar_desa($desa['logo']) and print('article-image w-full object-cover object-center') ?> mx-auto">
  </a>
  <div class="article-caption space-y-3 pt-3">
    ...
    <ul class="flex flex-wrap text-sm space-x-4">
      <li class="inline-flex items-center">
        <span data-feather="calendar" class="icon icon-sm text-secondary mr-2"></span>
        <span><?= tgl_indo($article['tgl_upload']) ?></span>
      </li>
      <li class="inline-flex items-center">
        <span data-feather="user" class="icon icon-sm text-secondary mr-2"></span>
