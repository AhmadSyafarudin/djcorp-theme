	...
	<div class="carosel swiper-location-carousel h-100 h-sm-auto">
		<div id="carouselDeNatra" class="carousel slide" data-ride="carousel">
			<div class="carousel-inner">
				...
				<div class="card mb-1 carousel-item <?= ($active == true)?"active":"" ?>" data-artikel="<?= $gambar['id']?>" <?php if ($slider_gambar['sumber'] != 3): ?> onclick="location.href='<?='artikel/'.buat_slug($gambar); ?>'" <?php endif; ?>>
					<img style="<?php if(config_item('fluid') == true) : ?>max-height:450px<?php else: ?>max-height:350px<?php endif; ?>" class="img-responsive cover img-thumbnail img-fluid opacity-100" src="<?= base_url().$slider_gambar['lokasi'].'sedang_'.$gambar['gambar']?>">
