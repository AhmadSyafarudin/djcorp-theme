...
<?php if($enable_customizer) : ?>
  ...
  <div 
    ...
    <div 
      ...
      aria-label="Kustomisasi warna"
      :class="{'hidden': customizerOpen, 'flex': !customizerOpen}"
      x-transition>
      <i data-feather="settings" class="icon icon-base inline animate-spin"></i>
    </div>
    <div 
      ...
      <?php foreach($colorScheme as $color) : ?>
        <span class="<?= "bg-[{$color['hex']}]" ?> w-10 h-10 inline-block cursor-pointer" data-value="<?= $color['name'] ?>" @click="() => {customizerOpen = false; ; changeColorScheme('<?= $color['name'] ?>')}" aria-label="Pilih warna <?= $color['name'] ?>">
