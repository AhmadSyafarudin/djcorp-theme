...
<div class="py-4 px-5">
  <h1 class="text-heading text-lg lg:text-2xl border-b border-dotted border-primary dark:border-white pb-2 title"><i data-feather="mail" class="inline-block mr-1"></i> Pengaduan</h1>
  <form action="<?= $search_action; ?>" method="get">
    <div class="flex gap-3 lg:w-7/12 flex-col lg:flex-row py-5">
      <button type="button" class="button button-primary flex-shrink-0" data-toggle="modal" data-target="#newpengaduan">Formulir Pengaduan</button>
      ...
      <?php if ($cari) : ?>
        <a href="<?= site_url('pengaduan'); ?>" class="button button-tertiary"><i class="fa fa-times"></i></a>
      <?php endif; ?>
    </div>
  </form>
  ...
  <?php if($pengaduan) : ?>
    <section class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <?php foreach($pengaduan as $key => $value) : ?>
        <div class="card p-5 border cursor-pointer" data-toggle="modal" data-target="#pengaduan<?= $value['id'] ?>">
          <dl>
            <dt class="font-bold lg:text-xl"><?= $value['judul']; ?></dt>
            <ul class="inline-flex flex-wrap gap-2 w-full items-center text-xs">
              <li class="inline-flex items-center"><i data-feather="calendar" class="inline-block w-4 mr-1 text-secondary"></i> <?= $value['created_at'] ?></li>
              <li class="inline-flex items-center"><i data-feather="user" class="inline-block w-4 mr-1 text-secondary"></i> <?= $value['nama'] ?></li>
              <li>
            ...
            <dd class="pt-2 flex flex-col lg:flex-row items-end justify-between gap-3">
              ...
              <span class="label label-<?= $value['jumlah'] > 0 ? 'success' : 'info'; ?> pull-right text-xs flex-shrink-0"><i class="fa fa-comments"></i> <?= $value['jumlah']; ?> Tanggapan</span>
            </dd>
          </dl>
        </div>
          ...
          <div class="modal-wrapper">
            <div class="modal-dialog">
              <div class="modal-content" style="border:none">
                <div class="modal-header overflow-hidden bg-secondary text-white">
                  <h4 class="modal-title inline-flex items-center gap-3"><i class="fa fa-folder-open"></i> <?= $value['judul'] ?></h4>
                </div>
                <div class="modal-body">
                  <div class="w-full py-2 space-y-2">
                    ...
                    <?php if ($value['foto']) : ?>
                      <img class="w-full" src="<?= base_url(LOKASI_PENGADUAN . $value['foto']); ?>">
                    <?php endif; ?>
                  </div>
                ...
                <div class="modal-footer">
                  <button type="button" class="button button-tertiary" data-dismiss="modal"><i class="fa fa-times"></i> Tutup</button>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="modal fade" id="newpengaduan" tabindex="-1" role="dialog" aria-labelledby="newpengaduan" aria-hidden="true">
    <div class="modal-wrapper">
      <div class="modal-dialog">
        <div class="modal-content" style="border:none">
          <div class="modal-header overflow-hidden bg-secondary text-white">
            <h4 class="modal-title inline-flex items-center gap-3 font-heading font-bold"><i class="fa fa-pencil fa-2x"></i> Buat Pengaduan Baru</h4>
          </div>
          <form action="<?= $form_action; ?>" method="POST" enctype="multipart/form-data">
              ...
              <div class="py-2">
                <div class="relative">
                  ...
                  <span class="absolute top-1/2 right-0 transform -translate-y-1/2 mr-1">
                    <button type="button" class="button button-info button-flat" id="file_browser"><i class="fa fa-search"></i></button>
                  </span>
                </div>
                <small>Gambar: png,jpg,jpeg</small><br>
                <br><img id="blah" src="#" alt="gambar pendukung di sini" class="max-w-full w-full hidden" />
              </div>
              <div class="flex">
            ...
            <div class="modal-footer">
              <button type="button" class="button button-tertiary pull-left" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i> Tutup</button>
              <button type="submit" class="button button-secondary pull-right"><i class="fa fa-pencil"></i> Kirim</button>
            </div>
          </form>
        </div>
      </div>
