...
<?php if(is_array($komentar) && $single_artikel['boleh_komentar']) : ?>
    ...
    <div class="py-3 divide-y">
      <?php foreach($comments as $comment) : ?>
        <div class="flex space-x-3 py-3">
          <img src="<?= base_url($this->theme_folder.'/'.$this->theme .'/assets/images/user.png') ?>" alt="user" class="w-12 h-12 rounded-full">
          <div class="space-y-1 flex flex-col">
            <span class="font-bold"><?= $comment['owner'] ?></span>
            <span class="inline-flex items-center text-sm text-slate-500 dark:text-slate-300"><i class="icon icon-sm mr-1 inline-block" data-feather="calendar"></i> <?= $comment['tgl_upload'] ?></span>
  ...
  <?php if($single_artikel['boleh_komentar'] == 1) : ?>
    <span class="block text-heading text-lg lg:text-xl">Kirim Komentar</span>
    ...
    <form action="<?= site_url('/add_comment/'.$single_artikel['id'])?>" method="POST"class="space-y-3"  id="kolom-komentar">
      ...
      <div class="flex flex-col lg:flex-row">
        ...
        <div class="w-full lg:w-7/12">
          <input type="text" class="form-input required" name="captcha_code" maxlength="6" value="<?= $notif['data']['captcha_code']; ?>" placeholder="Isikan kode captcha yang muncul">
        </div>
      </div>
      <button type="submit" class="button button-secondary">Kirim Komentar <span class="fa fa-send ml-2"></span></button>
    </form>
  <?php endif ?>
